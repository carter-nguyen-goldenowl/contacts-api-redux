...
import { searchContacts } from "../store/actions/contactActions";
import ViewModal from "./ViewModal";
import { viewContacts } from "../store/actions/contactActions";
...
export default function Contacts() {
    ...
    const hanledSearchContacts = () => {
      dispatch(searchContacts(searchData));
    }

    const handleViewData = (id) =>{
      dispatch(viewContacts(id));
    }
        ...
        <div className='py-6 md:py-12'>
          <div className="mb-3 xl:w-96 ml-6 mt-6">
            <div className="input-group relative flex flex-wrap items-stretch w-full mb-4">
              <input type="search" value={searchData} onChange={(e) => setSearchData(e.target.value)} className="form-control relative flex-auto min-w-0 block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" placeholder="Search" aria-label="Search" aria-describedby="button-addon2"/>
                  <button onClick={hanledSearchContacts}
                ...
                <div className="py-2 inline-block min-w-full sm:px-6 lg:px-8">
                  <div className="overflow-x-auto">
                    <table className="min-w-full">
                      ...
                      <tbody>
                        {contacts.map(contact => (
                          <tr className="border-b" key={contact.id} >
                            ...
                            <td>
                            <button onClick={(e) => {setOpenViewModal(true); handleViewData(contact.id)}} data-bs-toggle='modal' data-bs-target='#ViewModal'
                                    className="px-6 py-2.5 bg-green-400 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-green-500 hover:shadow-lg focus:bg-green-500 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-green-600 active:shadow-lg transition duration-150 ease-in-out" ><i className="fas fa-eye fa-xs"></i></button>
                      ...
                      {openEditModal && <EditModal editData={editData}/>}
